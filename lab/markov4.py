...
class MarkovModel:
    ...
    def _build_transition_graph(self) -> nx.DiGraph:
        ...
        for state in self.states:
            G.add_node(state.name, is_temporary=state.is_temporary, is_absorbing=state.is_absorbing)
            for transition in state.transitions:
                G.add_edge(state.name, transition["to_state"].name)
        return G

    def run(self, cycles: int, params: Dict, half_cycle_correction: bool = False, cohort: bool=False) -> None:
        """
        运行模型
        :param cycles: 模拟周期数
        :param params: 模型参数
        :param half_cycle_correction: 是否使用半周期矫正
        ...
        for t in range(cycles):
            ...
            if half_cycle_correction:
                # 半周期矫正：使用周期内平均状态
                avg_state = (current_state + next_state) / 2
            else:
            ...
            for i, state in enumerate(self.non_temporary_states):
                ...
                cycle_cost += avg_state[idx] * discounted_cost
                cycle_utility += avg_state[idx] * discounted_utility
                if not half_cycle_correction and t == cycles - 1:
                    # 注意不是半周期矫正的情况最终状态的成本和效用需要单独计算
                    last_cost += next_state[idx] * discounted_cost
                    last_utility += next_state[idx] * discounted_utility

            stage_costs.append(cycle_cost)
            stage_utilitys.append(cycle_utility)

            # last_cost、last_utility 仅在最后一次转移有值
            if not half_cycle_correction and t == cycles - 1:
                stage_costs.append(last_cost)
                stage_utilitys.append(last_utility)
    ...
    def visualize_transition_graph(self):
        """可视化状态转移图"""
        plt.figure(figsize=(12, 8))
        pos = nx.spring_layout(self.transition_graph)
        for n, d in self.transition_graph.nodes(data=True):
            print(n, d)
        # 区分节点类型
        temporary_nodes = [n for n, d in self.transition_graph.nodes(data=True) if d["is_temporary"]]
        non_temporary_nodes = [n for n, d in self.transition_graph.nodes(data=True) if
                               not d["is_temporary"] and not d["is_absorbing"]]
        absorbing_nodes = [n for n, d in self.transition_graph.nodes(data=True) if d["is_absorbing"]]

        # 绘制节点
        nx.draw_networkx_nodes(self.transition_graph, pos, nodelist=non_temporary_nodes,
                               node_color='lightblue', node_size=1000)
        nx.draw_networkx_nodes(self.transition_graph, pos, nodelist=temporary_nodes,
                               node_color='pink', node_size=800, node_shape='d')
        nx.draw_networkx_nodes(self.transition_graph, pos, nodelist=absorbing_nodes,
                               node_color='red', node_size=1000)

        # 绘制边
        nx.draw_networkx_edges(self.transition_graph, pos, width=1.0, alpha=0.7)

        # 绘制标签
        nx.draw_networkx_labels(self.transition_graph, pos)
...
if __name__ == "__main__":
    ...
    model = MarkovModel(
        ...
        initial_distribution={"Healthy": 100.0, "Disease": 0.0, "Death": 0.0, "Treatment": 0.0, "Diagnosis": 0.0}
    )

    # 可视化状态转移图
    model.visualize_transition_graph()

    # 运行模型
    model.run(cycles=10, params=params, half_cycle_correction=False, cohort=True)

    # 分析结果
    print(f"每个时间步的成本与效用：\n{model.results['stage_costs']}\n{model.results['stage_utilitys']}")
